...
const ALLOWED_FILES = [
	...
	APP_FOLDER + "/images/*.jpeg",
	APP_FOLDER + "/images/*.png",
	APP_FOLDER + "/images/*.svg",
	APP_FOLDER + "/images/*.ico",
	APP_FOLDER + "/polices/*.woff2",
	APP_FOLDER + "/polices/*.ttf",
	APP_FOLDER + "/polices/*.woff",
	APP_FOLDER + "/js/lib/flatpickr.mjs",
	APP_FOLDER + "/js/lib/flatpickr.min.css",
	APP_FOLDER + "/js/lib/mammoth.browser.min.js",
	APP_FOLDER + "/js/_locales/*.json",
	APP_FOLDER + "/js/_locales/flatpickr-*.mjs",
];

// Précompile les motifs (wildcards '*' -> regex) pour vérification rapide
const ALLOWED_PATTERNS = ALLOWED_FILES.map((p) => {
	if (p.includes("*")) {
		const escaped = p.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*");
		return new RegExp("^" + escaped + "$");
	}
	return p; // string exact match
});
...
const server = http.createServer((request, response) => {
	...
	// Vérification si le fichier demandé est autorisé (supporte les wildcards)
